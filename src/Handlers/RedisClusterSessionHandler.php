...
{
    protected RedisCluster $cluster;
    ...
    public function read(string $id): string|false
    {
        $value = $this->retry(function () use ($id) {
            return $this->cluster->get($this->prefix . $id);
        });
    ...
    public function write(string $id, string $data): bool
    {
        ...
        if ($this->ttl > 0) {
            $result = $this->retry(function () use ($key, $data) {
                return $this->cluster->setex($key, $this->ttl, $data);
            });
        } else {
            $result = $this->retry(function () use ($key, $data) {
                return $this->cluster->set($key, $data);
            });
        }
    ...
    public function destroy(string $id): bool
    {
        $this->retry(function () use ($id) {
            return $this->cluster->del($this->prefix . $id);
        });
